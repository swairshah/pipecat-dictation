...
def is_ydotool_available():
    ...
    try:
        ...
        result = subprocess.run(
            ["ydotool", "key", "--help"], capture_output=True, check=False, timeout=1
        )
...
class WindowController:
    ...
    def save_cache(self):
        ...
        try:
            ...
            data = {
                "windows": {name: info.to_dict() for name, info in self.window_map.items()},
    ...
    def focus_window(self, name: Optional[str] = None) -> bool:
        ...
        if self.platform in ["linux_wayland", "linux_x11"] and self.has_ydotool:
            try:
                subprocess.run(
                    ["ydotool", "mousemove", str(window.position[0]), str(window.position[1])],
                    check=False,
                )
                time.sleep(0.1)
    ...
    def send_keystrokes_to_window(
        self, text: str, window_name: Optional[str] = None, restore_mouse: bool = False
    ):
    ...
    def send_key_to_window(
        self, key: str, window_name: Optional[str] = None, restore_mouse: bool = False
    ):
    ...
    def send_keystrokes(self, text: str):
        ...
        if self.platform in ["linux_wayland", "linux_x11"] and self.has_ydotool:
            ...
            # Add delay after newlines to match ydotool behavior
            for char in text:
                self.keyboard_controller.type(char)
                time.sleep(0.02)  # 20ms delay after enter, matching ydotool
    ...
    def send_key(self, key: str):
        ...
        if self.platform in ["linux_wayland", "linux_x11"] and self.has_ydotool:
            ...
            if ydotool_key:
                try:
                    subprocess.run(
                        ["ydotool", "key", ydotool_key], capture_output=True, check=False
                    )
                except:
                    pass
        else:
    ...
    def list_windows(self):
        ...
        sorted_windows = sorted(
            self.window_map.items(), key=lambda x: x[1].last_used or 0, reverse=True
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Window control utility with memory management")
    ...
    add_parser.add_argument(
        "-w", "--wait", type=int, default=3, help="Seconds to wait before capture (default: 3)"
    )
    ...
    focus_parser.add_argument(
        "name", nargs="?", help="Window name (optional, defaults to last used)"
    )
